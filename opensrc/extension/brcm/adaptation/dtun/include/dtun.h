...
typedef enum {
    ...
    DTUN_METHOD_DM_FETCH_REMOTE_DI_INFO,
    DTUN_METHOD_DM_SET_AV_STREAMING,
    ...
    DTUN_SIG_DM_FETCH_REMOTE_DI_INFO,
    DTUN_SIG_DM_FETCH_REMOTE_DI_REC,
    DTUN_SIG_DM_AMP_STATE_CHANGED,

    /* AV signals */
    DTUN_SIG_AM_AV_EVENT,

   	/* Dual-HF Signals */
	DTUN_SIG_AG_SECOND_CONN_STATUS,
    DTUN_SIG_AG_SLC_UP,
...
#ifdef S1_JPN_NTT
#define DTUN_DEFAULT_DEV_NAME "SC-02B"
#else
#define DTUN_DEFAULT_DEV_NAME "GalaxyS+"
#endif
...
/* comment out this line disable local socket implementation */
#define BTLIF_LOCAL_SERVER_ADDR "brcm.bt.btlif" 

/* use tcp sockets for native & standalone test mode */
#if defined(LINUX_NATIVE) || defined(BTL_IF_TEST_ENABLE) || defined (DTUN_STANDALONE)
#undef BTLIF_LOCAL_SERVER_ADDR
#endif

#ifdef BTLIF_LOCAL_SERVER_ADDR
#define DTUN_SERVER_ADDR BTLIF_LOCAL_SERVER_ADDR

/* local server name format: brcm.bt.btlif.port_num */
#define BTLIF_MAKE_LOCAL_SERVER_NAME(name_new, name_old, port_num) \
    do { \
        name_new[0] = 0; \
        if(name_old && strlen(name_old) < UNIX_PATH_MAX - 10) \
        { \
            sprintf(name_new, "%s.%d", name_old, port_num); \
            debug("BTLIF_MAKE_LOCAL_SERVER_NAME return name: %s\n", name_new); \
        } \
        else debug("BTLIF_MAKE_LOCAL_SERVER_NAME failed, the name is null or too long!\n"); \
    } while(0)
#else
...
} tDTUN_METHOD_DM_FETCH_REMOTE_DI_INFO;

/* Set AV Streaming flag */
typedef struct
{
    tDTUN_HDR           hdr;
    boolean             av_streaming;
} tDTUN_METHOD_DM_SET_AV_STREAMING;
...
/* data type for DTUN_SIG_DM_FETCH_REMOTE_DI_INFO */
typedef struct
{
    tDTUN_HDR           hdr;
	bdaddr_t            remote_addr;
    uint16_t            rec_num;
    uint16_t            status;
} tDTUN_SIG_DM_FETCH_REMOTE_DI_INFO;

/* data type for DTUN_SIG_DM_FETCH_REMOTE_DI_REC */
#define DTUN_DI_MAX_ATTR_LEN 128
typedef struct
{
    tDTUN_HDR           hdr;
    uint16_t            handle;
    ...
    uint16_t            product_id;
    uint16_t            version;
    char                client_executable_url[DTUN_DI_MAX_ATTR_LEN];
    char                service_description[DTUN_DI_MAX_ATTR_LEN];
    char                documentation_url[DTUN_DI_MAX_ATTR_LEN];
} tDTUN_SIG_DM_FETCH_REMOTE_DI_REC;
...
typedef struct
{
    ...
    uint32_t            app_id;
    uint32_t            amp_state;
} tDTUN_SIG_DM_AMP_STATE_CHANGED;
...
typedef struct
{
    ...
    uint8_t             status; 
	bdaddr_t            peer_addr;
} tDTUN_SIG_AG_SECOND_CONN_STATUS;
...
typedef union
{
    ...
    tDTUN_METHOD_DM_RESOLVE_NAME         resolve_name;
    ...
    tDTUN_METHOD_DM_FETCH_REMOTE_DI_INFO fetch_remote_di_info;
    tDTUN_METHOD_DM_SET_AV_STREAMING    set_av_streaming;
...
typedef union
{
    ...
    tDTUN_SIG_DM_AMP_STATE_CHANGED     amp_state_changed;

	tDTUN_SIG_AG_SECOND_CONN_STATUS	  ag_conn_up;
    tDTUN_SIG_AG_SLC_UP               ag_slc_up;
